...
export class DocumentsController {
    ...
    public async search(req: Request, res: Response): Promise<Response>{
        ...
        if(req && req.params && req.params.search &&( searchValue = req.params.search)){
            try {
                searchResult = await pool.query("SELECT content, private, title, name, notes.id AS id FROM notes INNER JOIN users ON notes.authorid = users.id WHERE private=FALSE AND (title LIKE '%' || $1 || '%' OR content LIKE '%' || $1 || '%' OR name LIKE '%' || $1 || '%')" , [searchValue])
            } catch (e) {
