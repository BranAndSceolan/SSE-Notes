...
// Test base route to return string
    describe('Base Route Test',  () => {
        ...
        it('user:login', async ()=>{
            ...
            chai.expect(resCreate.status).to.equal(200)
            testResult = ( testResult && res.status == 200 && resCreate.status == 200)
        })
        ...
        chai.expect(resCreate.status).to.equal(200)
        testResult = ( testResult && resCreate.status == 200)
        })
        ...
        it ('documents:get. (own public note) should return 200', async ()=>{
            ...
            chai.expect(res.status).to.equal(200)
            chai.expect(res.body.title).to.equal("This should succeed")
            testResult = (testResult && res.status == 200 && res.body.title == "This should succeed")
        })
        ...
        it ('documents:get. (own private note) should return 200', async ()=>{
            ...
            chai.expect(res.status).to.equal(200)
            chai.expect(res.body.title).to.equal("This should succeed a second time")
            testResult = (testResult && res.status == 200 && res.body.title == "This should succeed a second time")
        })
        ...
        it ('user:delete. should return 200 and delete as well as log out user', async ()=>{
            ...
            const resReg = await chai.request(app).post('/api/user/register').send({
                name:	username,
                password:	"picket lock singer dread"
            })
